...
export function buildSchedulerTask(ctx: SchedulerContext): string {
  ...
  const lines: string[] = [
    ...
    isDebate && ctx.debateFlavor === "strict"
      ? `- 严格轮次模式：参会人按下标循环发言（${ctx.roles.length === 2 ? "A → B → A → B" : "A → B → C → A → B → C"}），不要插队；host 仅在用户介入或卡壳时出场；用户刚发言后挑下标离上轮最远的那位。但若上一句 host 明确点名某位参会人（见上面的硬规则），则按点名走，这是 strict 模式下唯一合法的破例`
      : "",
